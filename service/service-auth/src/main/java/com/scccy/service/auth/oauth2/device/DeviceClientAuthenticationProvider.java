...
public final class DeviceClientAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        ClientAuthenticationMethod clientAuthenticationMethod = deviceClientAuthentication.getClientAuthenticationMethod();
        // 只支持公共客户端，其他返回null
        ...
        // 校验客户端
        if (registeredClient != null && !registeredClient.getClientAuthenticationMethods().contains(clientAuthenticationMethod)) {
            throwInvalidClient("authentication_method");
        }
        if (log.isTraceEnabled()) {
            log.trace("Authenticated device client");
        }
        return new DeviceClientAuthenticationToken(registeredClient, clientAuthenticationMethod, null);
    }
